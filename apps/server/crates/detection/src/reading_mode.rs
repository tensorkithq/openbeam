...
impl ReadingMode {
    ...
    pub fn start(
        ...
        if loaded.is_empty() {
            tracing::warn!("[READING] No verses loaded for {book_name} {chapter}:{start_verse}");
            return;
        }
    ...
    pub fn resume(&mut self) {
        if !self.verses.is_empty() {
            ...
            tracing::info!("[READING] Resumed at: {} {}:{verse}", self.book_name, self.chapter);
        }
    }
    ...
    pub fn deactivate(&mut self) {
        if self.active || !self.verses.is_empty() {
            tracing::info!("[READING] Deactivated (verses cleared)");
        }
        self.active = false;
        self.verses.clear();
        self.accumulated_text.clear();
    ...
    pub fn check_chapter_command(&self, text: &str) -> Option<ChapterChange> {
        ...
        if trimmed == "next chapter" || trimmed == "next chapter." {
            ...
            tracing::info!("[READING] 'Next chapter' command detected → chapter {new_chapter}");
        ...
        if trimmed == "previous chapter" || trimmed == "previous chapter." {
            if self.chapter > 1 {
                let new_chapter = self.chapter - 1;
                tracing::info!("[READING] 'Previous chapter' command detected → chapter {new_chapter}");
                return Some(ChapterChange {
                    book_number: self.book_number,
                    book_name: self.book_name.clone(),
                    new_chapter,
        ...
        tracing::info!("[READING] Chapter change command detected: chapter {chapter_num}");
        Some(ChapterChange {
            book_number: self.book_number,
            book_name: self.book_name.clone(),
    ...
    pub fn check_transcript(&mut self, text: &str) -> Option<ReadingAdvance> {
        ...
        if self.last_match_time.elapsed().as_millis() > READING_MODE_TIMEOUT_MS
            && self.active
        {
            tracing::info!("[READING] Timeout — pausing (toggle still on, verses retained)");
            self.active = false;
        }
    ...
    fn check_verse_number_reference(&mut self, text: &str) -> Option<ReadingAdvance> {
        ...
        if trimmed == "next" || trimmed == "next." || trimmed == "next verse"
            ...
            let next_idx = self.current_index + 1;
            if next_idx < self.verses.len() {
                tracing::info!("[READING] 'Next' command detected");
                return self.advance_to(next_idx);
            }
            return None;
        }
        ...
        if trimmed == "previous verse" || trimmed == "previous verse."
            ...
            if self.current_index > 0 {
                ...
                tracing::info!("[READING] 'Previous' command detected");
        ...
        for (idx, v) in self.verses.iter().enumerate() {
            if v.verse_number == verse_num {
                tracing::info!("[READING] Verse number reference detected: verse {verse_num}");
    ...
    fn advance_to(&mut self, index: usize) -> Option<ReadingAdvance> {
        ...
        tracing::info!("[READING] Advanced to: {reference}");
