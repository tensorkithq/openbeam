...
/// `emit_event` which maps to WebSocket broadcast. Backend-bound commands
...
mod tests {
    ...
    fn frontend_commands_never_invoke_backend() {
        ...
        for cmd in &frontend_cmds {
            ...
            assert_eq!(sink.backend_count(), 0, "{cmd} should not call invoke_backend");
    ...
    fn backend_commands_never_emit_events() {
        ...
        for cmd in &backend_cmds {
            ...
            assert_eq!(sink.backend_count(), 1, "{cmd} should call invoke_backend once");
        }
    }
}
